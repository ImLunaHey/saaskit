...
export default function TeamSwitcher({ teams, user, className }: TeamSwitcherProps) {
  const [open, setOpen] = React.useState(false);
  const [showNewTeamDialog, setShowNewTeamDialog] = React.useState(false);
  const t = useI18n();
  const groups = [
    {
      label: t('teamSwitcher.personalAccount'),
      teams: [
        {
          label: user.username,
          value: 'personal',
    ...
    <Dialog open={showNewTeamDialog} onOpenChange={setShowNewTeamDialog}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[200px] p-0">
          <Command>
            <CommandList>
              <CommandInput placeholder={t('teamSwitcher.searchTeam')} />
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('teamSwitcher.createTeam')}</DialogTitle>
